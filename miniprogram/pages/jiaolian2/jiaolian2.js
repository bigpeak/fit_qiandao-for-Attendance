...
const cont = db.collection('jiaolian2');
...
Page({                                             //定义页面数据
  ...
  onLoad: function() {                             //默认加载获取用户信息
    ...
    if (!wx.cloud) {
      ...
      return
        ...
        // 查询当前用户所有的 counters
        db.collection('jiaolian2').count().then(res =>{
  ...
  onAdd:function() {
        ...
        db.collection('jiaolian2').where({_openid:app.globalData.openid}).get({
          success:res =>{
